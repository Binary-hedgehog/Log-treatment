'''
Модуль для различных математических функций,
констант, коэффициентов
'''
from math import atan, pi, sin, cos, sqrt, acos, acosh, asinh, cosh, sinh
...
compas_point = [55.739776, 37.787815, 187.376,
                2844181.913, 2205204.968, 5248340.243]
compas_point_op14 = [55.740273, 37.788794, 184.963,
                     2844107.395708284, 2205225.103510066, 5248370.201976600]
tatarskaya_point = [55.736411, 37.636278, 185,
                    2850248.945160372, 2197863.567382474, 5248128.113947969]
svmc1_point = [55.5366, 38.1772, 128.646,
               2843783.41532259, 2236005.047984719, 5235524.735569037]
svmc2_point = [55.53907, 38.17202, 128.711,
               2843807.334413563, 2235607.8317871485, 5235680.400647452]
onp2_point = [55.53728, 38.17549, 131.331,
              2843802.272020579, 2235882.532438007, 5235569.790714532]
syr_idlib_point = [35.9332921390998, 36.63599744144252, 442,
                   4149193.679766917, 3085509.6923088073, 3722460.350054406]
syr_hmeimim_point = [35.41371890952971, 35.94863134703012, 48,
...
def spline(x0, x1, x2, x3, x4, y0, y1, y2, y3, y4, x):
    ...
    c2 = (k2-k1*h2/(2*h1+2*h2) - k3*h3/(2*h3+2*h4)) / (
        2*h2 + 2*h3 - h2**2/(2*h1+2*h2) - h2*h3/(2*h3+2*h4))
...
def spline_v3(x0, x1, x2, x3, x4, y0, y1, y2, y3, y4, x):
    ...
    c3 = (k2-k1*h2/(2*h2+2*h1) - k3*h3/(2*h3+2*h4)) / (
        2*h2 + 2*h3 - h2**2/(2*h1+2*h2) - h3**2/(2*h3+2*h4))
...
def approx_linar(vvod, time, time0):
    ...
    for i in time:
        ...
        # vivod[ii] = spline(time0[a-2],time0[a-1],time0[a],time0[a+1],
        #                time0[a+2],vvod[a-2],vvod[a-1],vvod[a],vvod[a+1],vvod[a+2],i)
...
def approx3(vvod1, vvod2, vvod3, time, time0):
    ...
    for i in time:
        ...
        vivod1[ii] = spline(time0[a - 2], time0[a - 1], time0[a],
                            time0[a + 1], time0[a + 2], vvod1[a - 2],
                            vvod1[a - 1], vvod1[a], vvod1[a + 1],
                            vvod1[a + 2], i)
        vivod2[ii] = spline(time0[a - 2], time0[a - 1], time0[a],
                            time0[a + 1], time0[a + 2], vvod2[a - 2],
                            vvod2[a - 1], vvod2[a], vvod2[a + 1],
                            vvod2[a + 2], i)
        vivod3[ii] = spline(time0[a - 2], time0[a - 1], time0[a],
                            time0[a + 1], time0[a + 2], vvod3[a - 2],
                            vvod3[a - 1], vvod3[a], vvod3[a + 1],
                            vvod3[a + 2], i)
        # print(vivod[ii]," ",ii)
        ii += 1
...
def from_bin_to_dec(x, val=1):
    # перевод из двоичного в десятичный с домножением на значение младшего бита
    # print(x)
    y = 0
...
def extrop_coor_with_vel(coor, vel, dt=1):
    # Экстрополяция координат скоростью
    proection = []
